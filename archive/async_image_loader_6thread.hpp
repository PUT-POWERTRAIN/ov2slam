#ifndef ASYNC_IMAGE_LOADER_6THREAD_HPP
#define ASYNC_IMAGE_LOADER_6THREAD_HPP
...
// Parallel async image loader with 6-thread decode
// 3 threads for left images, 3 threads for right images
class AsyncImageLoader6Thread {
public:
    struct ImagePair {
        double timestamp;
        cv::Mat left;
    ...
    AsyncImageLoader6Thread(const std::string& left_dir,
        ...
        , stop_requested_(false)
    {
        ...
        left_thread1_ = std::thread(&AsyncImageLoader6Thread::leftWorker1, this);
        left_thread2_ = std::thread(&AsyncImageLoader6Thread::leftWorker2, this);
        left_thread3_ = std::thread(&AsyncImageLoader6Thread::leftWorker3, this);
        right_thread1_ = std::thread(&AsyncImageLoader6Thread::rightWorker1, this);
        right_thread2_ = std::thread(&AsyncImageLoader6Thread::rightWorker2, this);
        right_thread3_ = std::thread(&AsyncImageLoader6Thread::rightWorker3, this);
    }

    ~AsyncImageLoader6Thread() {
        stop_requested_ = true;
        cv_left1_.notify_all();
        cv_left2_.notify_all();
        cv_left3_.notify_all();
        cv_right1_.notify_all();
        cv_right2_.notify_all();
        cv_right3_.notify_all();
        if (left_thread1_.joinable()) left_thread1_.join();
        if (left_thread2_.joinable()) left_thread2_.join();
        if (left_thread3_.joinable()) left_thread3_.join();
        if (right_thread1_.joinable()) right_thread1_.join();
        if (right_thread2_.joinable()) right_thread2_.join();
        if (right_thread3_.joinable()) right_thread3_.join();
    }
    ...
    void addFrame(double timestamp, const std::string& img_name, size_t idx) {
        ...
        {
            std::lock_guard<std::mutex> lock(mtx_left3_);
            left_queue3_.push(task);
        }
        cv_left3_.notify_one();
        ...
        {
            std::lock_guard<std::mutex> lock(mtx_right3_);
            right_queue3_.push(task);
        }
        cv_right3_.notify_one();
    }
...
private:
    ...
    void leftWorker3() {
        while (!stop_requested_) {
            ...
            {
                std::unique_lock<std::mutex> lock(mtx_left3_);
                cv_left3_.wait(lock, [this] {
                    return !left_queue3_.empty() || stop_requested_;
                });

                if (stop_requested_) break;
                if (left_queue3_.empty()) continue;

                task = left_queue3_.front();
                left_queue3_.pop();
            }
            ...
            if (frame->left.empty()) {
                std::cerr << "Left3 failed: " << task.img_name << std::endl;
                continue;
            }
    ...
    void rightWorker3() {
        while (!stop_requested_) {
            ...
            {
                std::unique_lock<std::mutex> lock(mtx_right3_);
                cv_right3_.wait(lock, [this] {
                    return !right_queue3_.empty() || stop_requested_;
                });

                if (stop_requested_) break;
                if (right_queue3_.empty()) continue;

                task = right_queue3_.front();
                right_queue3_.pop();
            }
            ...
            if (frame->right.empty()) {
                std::cerr << "Right3 failed: " << task.img_name << std::endl;
                continue;
            }
    ...
    std::thread left_thread3_;
    std::queue<LoadTask> left_queue3_;
    std::mutex mtx_left3_;
    std::condition_variable cv_left3_;
    ...
    std::thread right_thread3_;
    std::queue<LoadTask> right_queue3_;
    std::mutex mtx_right3_;
    std::condition_variable cv_right3_;
...
#endif // ASYNC_IMAGE_LOADER_6THREAD_HPP
